...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
        ...
        <a href="{{ route('authors.create') }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-5 rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300">
        ...
        @forelse($authors as $author)
            <div class="author-card bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col">
                <div class="p-6 flex-grow">
                    <div class="flex items-center gap-6 mb-4">
                        ...
                        <div class="flex-shrink-0">
                            <img src="{{ $author->photo ? asset('storage/' . $author->photo) : 'https://placehold.co/100x100/e2e8f0/a0aec0?text=No+Photo' }}" alt="Photo of {{ $author->name }}" class="author-photo rounded-full object-cover">
                        </div>
                        {{-- Info Penulis --}}
                        <div class="flex-grow">
                    ...
                    {{-- Biografi Penulis --}}
                    <div>
                        <p id="bio-{{ $author->id }}" class="author-bio text-gray-500 text-sm mt-1 line-clamp-2">{{ $author->bio ?: 'No biography available.' }}</p>
                        {{-- Tombol Read More hanya muncul jika bio panjang --}}
                        @if(strlen($author->bio) > 100)
                            <button onclick="toggleBio({{ $author->id }})" class="text-xs text-indigo-600 hover:underline mt-2 font-semibold">Read More</button>
                        @endif
                    </div>
                        ...
                        <div id="book-list-{{ $author->id }}" class="book-list space-y-3">
                            @forelse($author->books as $book)
                                <div class="flex items-center gap-3">
                                    <img src="{{ $book->cover_image ? asset('storage/' . $book->cover_image) : 'https://placehold.co/40x60/e2e8f0/a0aec0?text=N/A' }}" class="w-10 h-14 object-cover rounded-sm shadow-sm">
                ...
                <div class="bg-gray-50 p-4 mt-auto flex justify-between items-center">
                    ...
                    <button onclick="toggleBooks({{ $author->id }})" class="text-sm text-indigo-600 hover:text-indigo-800 font-semibold">
                        View All Books
                    </button>
                    @else
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('authors.edit', $author->id) }}" class="text-gray-400 hover:text-blue-500 transition-colors" title="Edit">
                            <i class="fas fa-pencil-alt"></i>
                        </a>
                        <form action="{{ route('authors.destroy', $author->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button onclick="return confirm('Are you sure?')" class="text-gray-400 hover:text-red-500 transition-colors" title="Delete">
