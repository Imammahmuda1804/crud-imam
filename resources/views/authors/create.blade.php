            ...
            <form action="{{ route('authors.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-8">
                    ...
                    <div class="md:col-span-1 flex flex-col items-center justify-center">
                        ...
                        <div id="image-preview-wrapper" class="w-48 h-48">
                            <div
                                class="file-input-placeholder w-full h-full bg-gray-50 border-2 border-dashed border-gray-300 rounded-full flex items-center justify-center text-center p-4 cursor-pointer">
                            ...
                            <img id="image-preview" src="" alt="Photo Preview"
                                class="hidden w-full h-full object-cover rounded-full shadow-md" />
                        </div>
                        <input type="file" name="photo" id="photo" class="hidden"
                            accept="image/png, image/jpeg, image/jpg">
                        <button type="button" onclick="document.getElementById('photo').click()"
                            class="mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-semibold">
                            Choose Photo
                        </button>
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="bio" id="bio" rows="6"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                placeholder="Write a short biography about the author...">{{ old('bio') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-12 pt-6 border-t border-gray-200 flex justify-end gap-4">
                    <a href="{{ route('authors.index') }}"
                        class="py-2 px-6 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">Cancel</a>
                    <button type="submit"
                        class="py-2 px-6 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all">Save
                        Author</button>
                </div>
            </form>
        </div>
